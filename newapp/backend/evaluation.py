...
PAVIA_UNIVERSITY_NAMES_2 = ["tree", "asphalt", "brick", "bitumen", "shadow", 'meadow', 'bare soil']
...
class HSIEvaluation(object):
    def __init__(self, param) -> None:
        ...
        noise_type = param['data']['noise_type']
        if data_sign == 'Indian':
            self.target_names = INDIAN_TARGET_NAMES
        elif data_sign == "PaviaU":
            if noise_type == 'diff_dataset':
                self.target_names = PAVIA_UNIVERSITY_NAMES_2
            else:
    ...
    def eval(self, y_test, y_pred_test, class_num, verbose=False):
        ...
        print("Eval values: oa=%s, aa=%s, kappa=%s" % (self.res['oa'], self.res['aa'], self.res['kappa']))
        return self.res  