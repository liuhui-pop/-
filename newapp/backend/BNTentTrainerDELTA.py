...
class BaseTrainer(object):
    ...
    def train(self, train_loader, unlabel_loader=None, test_loader=None):
        ...
        for epoch in range(epochs):
            ...
            _push = self.params.get("_push_event")
            _job_id = self.params.get("_job_id")
            if _push and _job_id:
                event = {"type": "epoch", "epoch": epoch + 1,
                        "loss": round(epoch_avg_loss.get_avg(), 6)}
                rd = recorder.record_data
                oa_list = rd.get("train_oa", {}).get("value", [])
                if oa_list:
                    event["oa"] = round(float(oa_list[-1]), 4)
                _push(_job_id, event)
            # 一定epoch下进行一次eval
    ...
    # ] 
        ...
        np.save(self.params['path_res'] + '_pred.npy', dat)
        # # plt.imshow(dat)
        # # plt.show()
    ...
    def save_classification_map(self, all_loader, dataloader, save_path_prefix):
        """
        用 all_loader 对全图所有像素预测，生成分类图。
        保存两个文件：
          - {save_path_prefix}_pred.npy  （像素级预测标签）
          - {save_path_prefix}_pred.jpg  （彩色分类图）
        所有模型训练/ctent 完成后自动调用。
        """
        try:
            import matplotlib
            matplotlib.use("Agg")
            ...
            # 颜色表（index=0 为背景黑色，index=1~N 对应各类别）
            COLORS = [
                [0,   0,   0  ],  # 0: 背景（黑色）
                [0,   0,   255],  # 1
                [255, 128, 0  ],  # 2
            ...
            self.net.eval()
            all_preds, all_xs, all_ys = [], [], []
 
            with torch.no_grad():
                for inputs, labels, x0, y0 in all_loader:
                    inputs = inputs.to(self.device)
                    outputs = self.net(inputs)
                    logits = outputs[0] if isinstance(outputs, tuple) else outputs
                    preds = np.argmax(logits.detach().cpu().numpy(), axis=1)
                    all_preds.extend(preds.tolist())
                    all_xs.extend(x0.tolist() if hasattr(x0, 'tolist') else list(x0))
                    all_ys.extend(y0.tolist() if hasattr(y0, 'tolist') else list(y0))
            ...
            for pred, x, y in zip(all_preds, all_xs, all_ys):
                pred_map[int(x), int(y)] = int(pred) + 1  # +1 还原类别编号（从1开始）
 
            # 保存 npy
            npy_path = save_path_prefix + '_pred.npy'
            ...
            for i in range(h):
                for j in range(w):
                    if labels_map[i, j] == 0:
                        color_map[i, j] = [0, 0, 0]  # 背景黑色
                    else:
                        cls = pred_map[i, j]
                        color_map[i, j] = COLORS[cls % len(COLORS)]
 
            # 保存 jpg（无边框、无坐标轴）
            jpg_path = save_path_prefix + '_pred.jpg'
            ...
            fig.add_axes(ax)
            ax.imshow(color_map)
            fig.savefig(jpg_path, dpi=100, bbox_inches='tight', pad_inches=0)
            plt.close(fig)
 
            print(f"[classification map] npy: {npy_path}")
            print(f"[classification map] jpg: {jpg_path}")
            return npy_path, jpg_path
 
        except Exception as e:
            import traceback
            print(f"[classification map] 生成失败: {e}")
            traceback.print_exc()
            return None, None
...
class SQSTrainer(BaseTrainer):
    def real_init(self):
        self.net = SQSTrainer(self.params).to(self.device)
        self.criterion = nn.CrossEntropyLoss()
        lr = self.train_params.get('lr', 0.001)
        wd = self.train_params.get('weight_decay', 5e-3)
