...
class Augment:
    ...
    def do(self,data,aug):
        return self.real_do(data,aug)
...
class XMaskAugment(Augment):
    ...
    def real_do(self, data) -> Tensor:
        ...
        return left, right
def fog_preprocess(inputs, fog_strength=0.7):
    """
    fog_strength: 0 表示不增强；1 表示强增强
    ...
    # gamma correction
    gamma = 1.0 + 0.5 * fog_strength  # 1.0~1.5
    inputs = torch.clamp(inputs, 0, 1)
    inputs = inputs ** (1.0 / gamma)

    # contrast boost
    contrast_factor = 1.0 + fog_strength  # 1.0~2.0
    inputs = inputs * contrast_factor - (contrast_factor - 1) / 2
    return torch.clamp(inputs, 0, 1)
class SWAugment(Augment):
    def __init__(self, params) -> None:
        super().__init__(params)

    def real_do(self,data,aug):
        b, s, h, w = data.shape
        x_aug = data.clone()
        x_weak = fog_preprocess(data,aug)
        drop_ratio = 0.05
        for i in range(b):
            if torch.rand(1).item() > 0.5:
                erase_size = int(min(h, w) * 0.1)
                x1 = torch.randint(0, h - erase_size, (1,))
                y1 = torch.randint(0, w - erase_size, (1,))
                x_aug[i, :, x1:x1+erase_size, y1:y1+erase_size] = 0
        
        num_drop = int(s * drop_ratio)
        drop_idx = torch.randperm(s)[:num_drop]
        x_aug[:, drop_idx, :, :] = 0
        if torch.rand(1).item() > 0.5:
            scale = 1 + 0.1 * (2 * torch.rand_like(x_aug) - 1)  # 在 ±10% 变化
            x_aug = x_aug * scale
            x_aug = torch.clamp(x_aug, 0.0, 1.0)

        if torch.rand(1).item() > 0.5:
            x_noise = torch.flip(x_weak, dims=[3])
        return x_weak, x_aug


def do_augment(params,data,aug):# 增强也有一系列参数呢，比如multiscale的尺寸、mask的大小、Gaussian噪声的参数等
    ...
    if params['type'] == 'SW':
        return SWAugment(params).do(data,aug)