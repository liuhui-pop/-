...
class massformer(nn.Module):
    def __init__(
        ...
        self.conv2d = nn.Sequential(
            nn.Conv2d(in_channels=808, out_channels=64, kernel_size=(3, 3)),
            nn.ReLU(),
        )
        self.flatten = nn.Flatten(1, 2)

        # self.nn = nn.Linear(?, dim)  # ← 你如果还用这层，需要明确输入维度；或删掉
        # torch.nn.init.xavier_uniform_(self.nn.weight)
        self.dropout = nn.Dropout(dropout)

        self.cls_token = nn.Parameter(torch.zeros(1, 1, dim))
        self.pos_embedding = None  # 动态创建
    ...
    def forward(self, x):
        x = self.conv3d(x)         # (B, 8, D, H, W)
        x = self.flatten(x)        # (B, 8*D, H, W)
        x = self.conv2d(x)         # (B, 64, H', W')
        x = rearrange(x, 'b c h w -> b (h w) c')  # (B, num_tokens, dim)

        max_token = self.maxpool(x)      # (B, reduced_tokens, dim)
        ...
        x = torch.cat((cls_token, x), dim=1)  # (B, num_tokens+1, dim)

        # 动态生成或更新 pos_embedding
        if self.pos_embedding is None or self.pos_embedding.shape[1] != x.shape[1]:
            self.pos_embedding = nn.Parameter(torch.randn(1, x.shape[1], x.shape[2], device=x.device))
