...
class SimAugment(Augment):
    def __init__(self,params) -> None:
        super().__init__(params)
        self.scale=params.get("patch",13)
    ...
    def real_do(self, data):
        ...
        n = 0
        for i in data:
            sim = compute_block_similarity(i,self.scale)
            po_data[n] = sim[0][0]
            ne_data[n] = sim[-1][0]
            n = n+1
        return po_data,ne_data


        


class DownSampleAugment(Augment): #降采样操作
    # 降采样
    def __init__(self, params) -> None:
        super().__init__(params)
    ...
    def real_do(self, data):
        x=F.interpolate(data,scale_factor=(1./self.scale,1./self.scale)) #按1/scale的比例进行缩小
...
def do_augment(params,data):# 增强也有一系列参数呢，比如multiscale的尺寸、mask的大小、Gaussian噪声的参数等
    ...
    if params['type'] == 'Sim':
        return SimAugment(params).do(data)