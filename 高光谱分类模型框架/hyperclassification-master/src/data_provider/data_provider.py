...
class DataSetIter(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        patch = self.base_img[start_x:start_x+2*self.margin+1 , start_y:start_y+2*self.margin+1,:] #获得patch，提取以(start_x, start_y)为起点，大小为(2*self.margin+1) * (2*self.margin+1)的补丁，并保存到变量patch中。
        if self.append_dim:
...
class HSIDataLoader(object):
    def __init__(self, param) -> None:
        ...
        self.patch_size = self.data_param.get('patch_size', 13) # n * n 13
        ...
        for r in range(margin, zeroPaddedX.shape[0] - margin):
            for c in range(margin, zeroPaddedX.shape[1] - margin):
                ...
                if temp_tr > 0: #train data
                    trainX_index2pos[trainIndex] = [start_x, start_y] #保存测试集的点位信息
                    trainIndex += 1
                elif temp_te > 0:
                    testX_index2pos[testIndex] = [start_x, start_y] #保存测试集的点位信息
                    testIndex += 1
                all_index2pos[patchIndex] =[start_x, start_y] #所有点位信息保存到all中
    ...
    def mean_var_norm(self, data):
        ...
        data = StandardScaler().fit_transform(data) #使用StandardScaler类对数据进行标准化处理，即将每个特征的均值调整为0，方差调整为1
        data = data.reshape(h, w, c)
        return data
...
if __name__ == "__main__":
    dataloader = HSIDataLoader({"data":{"data_path_prefix":'../../data', "data_sign": "Indian",
        "data_file": "Indian_20", "use_dump":False}})
