...
import random
def randomnum(num,data):
    lenth=data.shape[0]-1
    positive_index = [random.randint(0, lenth) for _ in range(num)]
    switch_index = []
    while len(switch_index)<num:
        nums = random.randint(0, lenth)
        if num not in positive_index:
            switch_index.append(nums)
    return positive_index,switch_index

def compute_similarity(vector1, vector2):
    similarity = compare_ssim(vector1, vector2,multichannel=True,data_range=vector2.max() - vector2.min()) 
    return similarity
def compute_block_similarity(positive,block_size): #positive 30*13*13
    positive_numpy = positive.numpy()
    height=positive.shape[1]
    width=positive.shape[2]
    positive_ex=torch.unsqueeze(positive, dim=0) #1*30*11*11
    similarities = []
    for i in range(0, height - block_size + 1):
        for j in range(0, width - block_size + 1):
            block = positive_ex[:,:,i:i+block_size, j:j+block_size]
            block_resize = F.interpolate(block,size=(height,width),mode='bicubic',align_corners=False)
            block_resize = torch.squeeze(block_resize,dim=0)
            blockre_numpy = block_resize.numpy()
            similarity = compute_similarity(blockre_numpy, positive_numpy)
            similarities.append((block_resize, similarity))
...
class SVMTrainer(SKlearnTrainer): #SVM的
    def __init__(self, params) -> None:
...
class RandomForestTrainer(SKlearnTrainer):  #用于训练和管理随机森林分类器模型的训练器
    def __init__(self, params) -> None:
...
class KNNTrainer(SKlearnTrainer): #用于训练和管理K近邻模型的训练器
    def __init__(self, params) -> None:
        super().__init__(params)
...
class BaseContraTrainer(object):
    ...
    def train(self, train_loader, unlabel_loader=None,test_loader=None):
        ...
        '''
        第一阶段使用原始data也过一遍backbone，然后去做CE loss。
        '''
        for epoch in range(pre_epochs): #先训练去标记unlabel
            self.net.train()
            epoch_avg_loss.reset()
            for i, (data, target) in enumerate(train_loader):
                ...
                if self.aug:
                    po_data,ne_data = do_augment(self.aug,data)
                    po_data = po_data.to(self.device)
                    ne_data = ne_data.to(self.device)
                    data = data.to(self.device)
                    outputs = self.net(data,po_data,ne_data)
                else:
        ...
        for epoch in range(contra_epochs):
            ...
            for i, (data, target) in enumerate(train_loader):
                target = target.to(self.device)
                # positive_index,switch_index = randomnum(10,data)
                # for i in range(len(positive_index)-1):
                #     sim = compute_block_similarity(data[positive_index[i]], 10)
                #     data[switch_index[i]] = sim[-1][0]
                
                label_batch=data.size(0)
                if self.use_unlabel:
                    unlabel_data,unlabel_target=self.get_next_unlabel()
                    data=torch.cat([data,unlabel_data],dim=0)
                    # positive_index,switch_index = randomnum(30,data)
                    # for i in range(len(positive_index)-1):
                    #     sim = compute_block_similarity(data[positive_index[i]], 10)
                    #     data[switch_index[i]] = sim[-1][0]
                ...
                #     left_data, right_data = do_augment(self.aug,data)
                #     left_data, right_data = [d.to(self.device) for d in [left_data, right_data]]
                    outputs = self.net(data,left_data,data)
                if self.aug:
                    po_data,ne_data = do_augment(self.aug,data)
                    po_data = po_data.to(self.device)
                    ne_data = ne_data.to(self.device)
                ...
                # print(outputs[0].size())   #[0]58*9 [1]58*64 [2]58*64
    ...
    def infoNCE(self, A_vecs, B_vecs, targets, temperature=15):
        ...
        '''
        A_vecs = torch.divide(A_vecs, torch.norm(A_vecs, p=2, dim=1, keepdim=True)) #对A归一化
        B_vecs = torch.divide(B_vecs, torch.norm(B_vecs, p=2, dim=1, keepdim=True)) #对B归一化
        matrix_logits = torch.matmul(A_vecs, torch.transpose(B_vecs, 0, 1)) * temperature # [batch, batch] each row represents one A item match all B 计算A和B的内积矩阵
        # tempa = matrix_logits.detach().cpu().numpy()
        # print("logits,", tempa.max(), tempa.min())
        matrix_softmax = torch.softmax(matrix_logits, dim=1) # softmax by dim=1，该矩阵的每一行表示一个 A_vecs 中的向量与所有 B_vecs 中的向量的匹配概率
        tempb = matrix_softmax.detach().cpu().numpy()
        # print("softmax,", tempb)
        # print("label,", targets)
        matrix_log = -1 * torch.log(matrix_softmax) #取对数并乘以 -1
        ...
        loss_nce = torch.sum(matrix_log * mask_matrix) / torch.sum(mask_matrix) # matrix_log 和 mask_matrix 之间的点乘，并将结果除以 mask_matrix 中非零元素的数量，得到最终的 InfoNCE 损失 loss_nce。
        return loss_nce
...
# with open('./params/indian_cross_param.json','r') as file:
