...
class HSINet(nn.Module):
    def __init__(self, params):
        ...
        self.pixel_patch_embedding = nn.Linear(conv2d_out, dim) #全连接

        self.local_trans_pixel = Transformer(dim=dim, depth=depth, heads=heads, dim_heads=dim_heads, mlp_dim=mlp_dim, dropout=dropout) #一个 Transformer 模型，用于对图像分块进行局部的自注意力计算
    ...
    def encoder_block(self, x):
        ...
        img = w * h
        x_pixel = self.conv2d_features(x_pixel) #输入图像先过卷积层
        ...
        #3. local transformer
        cls_tokens_pixel = self.cls_token_pixel.expand(x_pixel.shape[0], -1, -1) #增加类别标记
        ...
        x_pixel = self.local_trans_pixel(x_pixel) #(batch, image_size+1, dim) #过Transformer

        logit_pixel = self.to_latent_pixel(x_pixel[:,0]) #batch,dim
